# Trialing daymet data to get daily estimates of percipitation and temperature for the Sonoma Valley
...
if(midge == TRUE){
	...
	napa <- st_read("../../WinegrapeRegions/selectRegions/SonomaValley.shp")
	...
	#napa shape file - not teh final one, but teh obe I got from Nacho ages ago. for TRIAL
	napa <- st_read("/home/faith/Documents/mnt/UBC/Wine/climate/RE_Winegrape_regions/selectRegions/SonomaValley.shp")
	napa1 <- napa[1] #select just one layer 
	#Raster files 
	rasterFiles <- list.files(recursive = TRUE, pattern = ".nc")
...
if(SaveMapFiles == TRUE){
	for(i in 1:(length(rasterFilesMax))){
		#i <- 1
		writeRaster(stack(rasterMaxCrop[[i]]), filename=paste0(paste0( "sonoma/tmax/tmax_",i+1979), ".nc"), overwrite=TRUE, format="CDF") # FAITH: update your directory here
		writeRaster(stack(rasterMinCrop[[i]]), filename=paste0(paste0( "sonoma/tmin/tmin_",i+1979), ".nc"), overwrite=TRUE, format="CDF") # FAITH: update your directory here
		writeRaster(stack(rasterMaxPrcp[[i]]), filename=paste0(paste0( "sonoma/prcp/prcp_",i+1979), ".nc"), overwrite=TRUE, format="CDF") # FAITH: update your directory here
	}
