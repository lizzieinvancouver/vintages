# Trialing daymet data to get daily estimates of percipitation and temperature for the North Coast (non Napa or Sonoma or mountains")
#Based on code made for teh Napa valley 

#for my project and Geoffs project

#started by Faith Jones Sep 24 2021 based on code from Sep 2021
...
if(midge == TRUE){
	...
	napa <- st_read("../../WinegrapeRegions/selectRegions/NorthCoast_nonNaSonMount.shp")
	...
	#napa shape file - not teh final one, but teh obe I got from Nacho ages ago. for TRIAL
	napa <- st_read("/home/faith/Documents/mnt/UBC/Wine/climate/RE_Winegrape_regions/selectRegions/NorthCoast_nonNaSonMount.shp")
	napa1 <- napa[1] #select just one layer 
	#Raster files 
	rasterFiles <- list.files(recursive = TRUE, pattern = ".nc")
...
if(SaveMapFiles == TRUE){
	for(i in 1:(length(rasterFilesMax))){
		#i <- 1
		writeRaster(stack(rasterMaxCrop[[i]]), filename=paste0(paste0( "nCoast/tmax/tmax_",i+1979), ".nc"), overwrite=TRUE, format="CDF") # FAITH: update your directory here
		writeRaster(stack(rasterMinCrop[[i]]), filename=paste0(paste0( "nCoast/tmin/tmin_",i+1979), ".nc"), overwrite=TRUE, format="CDF") # FAITH: update your directory here
		writeRaster(stack(rasterMaxPrcp[[i]]), filename=paste0(paste0( "nCoast/prcp/prcp_",i+1979), ".nc"), overwrite=TRUE, format="CDF") # FAITH: update your directory here
	}
