...
## Fit Stan model
fit1 <- stan("project_winequality/stan/threephases.stan",
             data = data.stan,
             iter = 2000,
             warmup = 1000,
             chains = 4)
...
names(fit1)[524:528] <- locs
names(fit1)[529:535] <- varieties

## View diagnostics
launch_shinystan(fit1)

## Save estimates
saveRDS(object = fit1, file = "project_winequality/output/posterior_threephases.RDS")

## Summarize posterior samples
summary(fit1, pars = c("base_rank", "a_location", "sigma_location", "a_variety", "sigma_variety", "sigma_rank", "b_precip1", "b_precip2", "b_precip3", "b_gdd1", "b_gdd2", "b_gdd3"))$summary[, "mean"]

pdf(file = "project_winequality/output/Results_threephases.pdf", onefile = TRUE)
